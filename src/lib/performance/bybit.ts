...
import {
  sanitizeBybitApiKeyInfo,
  type BybitApiKeyInfo,
} from "./bybit-identity";

export type { BybitApiKeyInfo } from "./bybit-identity";
...
export async function fetchBybitAccountIdentity(): Promise<BybitAccountIdentity> {
  ...
  return {
    ...
    apiKey: sanitizeBybitApiKeyInfo(
      response.result as BybitApiKeyInfo & Record<string, unknown>
    ),
  };
}
