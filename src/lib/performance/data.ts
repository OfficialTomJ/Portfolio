...
function toPublicTrade(document: PublishedPerformanceTrade): PerformanceTrade {
  return {
    id: document.id,
...
export async function getLivePerformanceDataset(): Promise<PerformanceDataset> {
  ...
  try {
    ...
    return {
      ...
      description: documents.length ? "Published closed trades" : "No published closed trades yet",
      inceptionAt: inceptionAt.toISOString(),
      asOf: asOf.toISOString(),
      trades: documents.map(toPublicTrade),
    };
  } catch (error) {
