...
import { getMasterclassOneForm } from "../graphql/queries";
import { updateMasterclassOneForm } from "../graphql/mutations";
const client = generateClient();
export default function MasterclassOneFormUpdateForm(props) {
  const {
    id: idProp,
    masterclassOneForm: masterclassOneFormModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = masterclassOneFormRecord
      ? { ...initialValues, ...masterclassOneFormRecord }
      : initialValues;
    setFull_name(cleanValues.full_name);
    setEmail(cleanValues.email);
    setTrading_experience(cleanValues.trading_experience);
    setTrading_skill(cleanValues.trading_skill);
    setDescription(cleanValues.description);
    setAdditional(cleanValues.additional);
    setErrors({});
  };
  const [masterclassOneFormRecord, setMasterclassOneFormRecord] =
    React.useState(masterclassOneFormModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getMasterclassOneForm.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getMasterclassOneForm
        : masterclassOneFormModelProp;
      setMasterclassOneFormRecord(record);
    };
    queryData();
  }, [idProp, masterclassOneFormModelProp]);
  React.useEffect(resetStateValues, [masterclassOneFormRecord]);
  const validations = {
    full_name: [],
    email: [],
    trading_experience: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          trading_skill: trading_skill ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateMasterclassOneForm.replaceAll("__typename", ""),
            variables: {
              input: {
                id: masterclassOneFormRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "MasterclassOneFormUpdateForm")}
      {...rest}
    >
      <TextField
        label="Full name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || masterclassOneFormModelProp)}
